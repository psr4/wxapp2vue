<template>
    <page>
        <table>      <thead>          <tr>              <th>店铺</th>              <!-- <th>详情</th> -->              <th>时间</th>              <th>金额</th>          </tr>      </thead>      <tbody>          <tr :key="index" v-for="(item,index) in list">              <td>{{item.store}}</td>              <!-- <td>会员消费100元</td> -->              <td>{{item.add_time}}</td>              <td>{{item.money}}</td>          </tr>      </tbody>  </table>
    </page>
</template>
<script>
...
const pageData = Page({
	...
	getPageData() {
		...
		util.getAjaxData('store_user_account', data).then((res) => {
