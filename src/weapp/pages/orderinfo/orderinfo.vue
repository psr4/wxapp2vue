<template>
    <div class="page">
        <!--pages/orderinfo/orderinfo.wxml--> <div src="/template/common.wxml" import="1" />  <div class="details">   <div class="detail">     <div class="detailid">       <div class="info">日期:{{item.add_time}}</div>       <div class="info">订单号:{{item.orderId}}</div>       <div class="info">订单状态:{{item.status_T}}</div>       <div class="info">收货地址:{{item.address_name}} ({{item.mobile}}) {{item.address}}</div>       <div class="info" v-if="(item.fahuo_time)">配送方式:{{item.userfree + ":" + item.freecode}}</div>       <div class="info">订单金额:{{item.order_sumPrice}}</div>     </div>     <div class="items">       <div class="item" :key="index" v-for="(list,index) in item.items">         <div class="pic">           <img :src="(list.img)"></img>         </div>         <div class="desc">           <div class="title">{{list.title}}</div>           <div class="info">单价:{{list.price}} 数量:{{list.quantity}}</div>           <div class="info">{{list.item_attr}} </div>         </div>       </div>     </div>   </div> </div>    <tabbar is="tabbar"></tabbar>  
    </div>
</template>
<script>
