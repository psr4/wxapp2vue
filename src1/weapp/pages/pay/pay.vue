<template>
    <div class="page">
        <!--pages/pay/pay.wxml-->  <div src="/template/common.wxml" import="1"/>    <div class="box">      <div class="title">联系人地址</div>      <div class="address" :data-id="(address.id)" v-if="(address)">        <div class="info">{{address.consignee}} ( {{address.mobile}} )</div>        <div class="addr">{{address.sheng}} {{address.shi}} {{address.qu}} {{address.address}}</div>      </div>      <navigator class="choose" url="/pages/address/address?method=chooseaddress">选择地址</navigator>  </div>      <!--view class="box">    <block wx:if="{{yunfei.freesum == 0}}">    <view class="title1">卖家承担运费</view>    </block>    <block wx:elif="{{yunfei.freesum > 0}}">  <view class="title1">选择配送方式:</view>   <picker-view class="select" indicator-style="height: 30px;" value="{{value}}" bindchange="bindChange1">      <picker-view-column>        <view class="items">平邮</view>        <view class="items">快递</view>        <view class="items">EMS</view>      </picker-view-column>    </picker-view>      </block>  </view-->    <div class="box">    <div class="title1">给卖家的留言:</div>     <textarea placeholder="选填，可以告诉卖家您对商品的特殊需求，如颜色、尺码等" @keyDown="textInput"/>  </div>    <div class="box">      <navigator url="/pages/coupon/coupon?method=choose">          <div class="title1">              <span> 使用优惠券:      {{discount?discount.name:"选择优惠券"}}</span>             </div>      </navigator>  </div>    <div class="box text-right">  <!--view class="tip">满199减50、满399减100（点击“下单完成支付”自动满减）</view-->    <div class="title1">合计:¥{{totalmoney}} + 运费:¥{{yunfei}} - 优惠¥{{yh}}</div>    <div class="title1">=¥{{totalmoney - yh + yunfei}}元</div>  </div>    <div class="box">    <button @click="gopay">下单完成支付</button>  </div>   <contact is="contact" :data="(contaction)"></contact>     <div style="height:1.2rem;"></div>  <tabbar is="tabbar"></tabbar>   
    </div>
</template>
<script>
