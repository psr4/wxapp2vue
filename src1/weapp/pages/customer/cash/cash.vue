<template>
    <div class="page">
        <table>      <thead>          <tr>              <th>店铺</th>              <th>时间</th>              <th>金额</th>          </tr>      </thead>      <tbody>          <tr :key="index" v-for="(item,index) in list">              <td>{{item.store}}</td>              <td>{{item.add_time}}</td>              <td>{{item.money}}</td>          </tr>      </tbody>  </table> 
    </div>
</template>
<script>
...
const pageData = Page({
	...
	getPageData() {
		...
		util.getAjaxData('store_user_cash', data).then((res) => {
