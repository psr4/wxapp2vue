<template>
    <div class="page">
        <!--pages/coupon/coupon.wxml-->  <div src="/template/common.wxml" import="1" />    <div class="tab" v-if="(method != 'choose')">    <div data-status="1" :class="('item '+(status == 1?'active':''))" @click="changetab">未使用</div>    <div data-status="2" :class="('item '+(status == 2?'active':''))" @click="changetab">已使用/已过期</div>  </div>    <div class="box">    <div :class="('coupon '+(((item.canuse == 1 || method != 'choose') && status == 1)?'active':''))" :catchtap="(method)" :data-index="(index)" v-if="(status == item.used)" :key="index" v-for="(item,index) in discount"    >      <div class="left">        <div class="name">{{item.type == 1?'现金券':'满减券'}}</div>        <div class="value"><span>￥</span>{{item.price}}</div>      </div>      <div class="right">        <div class="manjian">{{item.type == 1?'立减 ￥'+item.price:'满'+item.usable+'减'+item.price}}</div>        <div class="tongyong">{{item.name}}</div>        <div class="time">{{item.stime}} - {{item.etime}}</div>      </div>    </div>  </div>       <contact is="contact" :data="(contaction)"></contact>     <tabbar is="tabbar" v-if="(method != 'choose')"></tabbar> 
    </div>
</template>
<script>
