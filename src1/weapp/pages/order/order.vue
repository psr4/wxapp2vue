<template>
    <div class="page">
        <div src="/template/common.wxml" import="1" />    <div class="tap">    <div data-status="0" :class="('item '+(status == 0?'active':''))" @click="changetap">全部</div>    <div data-status="1" :class="('item '+(status == 1?'active':''))" @click="changetap">待付款</div>    <div data-status="2" :class="('item '+(status == 2?'active':''))" @click="changetap">待发货</div>    <div data-status="3" :class="('item '+(status == 3?'active':''))" @click="changetap">待收货</div>    <div data-status="4" :class="('item '+(status == 4?'active':''))" @click="changetap">已完成</div>  </div>    <div :class="('details '+((status == item.status || status == 0)?'show':'hide'))" :key="index" v-for="(item,index) in detail" >    <div class="detail">      <div class="id">订单号：{{item.orderId}}</div>      <div class="status">{{item.status_T}}</div>    </div>    <div class="items">      <div class="item" :key="index" v-for="(list,index) in item.items">        <div class="pic">          <img :src="(list.img)"></img>        </div>        <div class="desc">          <div class="title">{{list.title}}</div>          <div class="xiangqing">{{list.item_attr}} </div>        </div>        <div class="info">          <div class="price">￥{{list.price}} </div>          <div class="amount"> {{list.quantity}}</div>        </div>      </div>      <div class="btns">        <div class="time">日期：{{item.add_time}}</div>        <button @click="controller" :data-status="(item.status)" :data-id="(item.orderId)" :data-rid="(item.id)">操作</button>      </div>    </div>  </div>     <contact is="contact" :data="(contaction)"></contact>     <div style="height:60px"></div>  <tabbar is="tabbar"></tabbar>             
    </div>
</template>
<script>
